 ...
 */
const compileTemplate = async (path) => handlebars.compile((await fs.readFile(path)).toString());
 ...
 * @param {string} path The path to a .hbs file.
 */
const registerPartialTemplate = async (name, path) => handlebars.registerPartial(name, await compileTemplate(path));
 ...
 */
const renderTemplate = async (path, data) => (await compileTemplate(path))(data);
